...
export default function ProfileView(props: {
  ...
  const handleDeletePost = (postid: number) => {
    ...
    if (props.loggedIn === false) {
      ...
      axios
        .delete("http://localhost:8080/user/deletepost/" + postid, headers)
        .then((res) => {
          console.log(res);
          if (res.status === 200) {
  ...
  const handleImageUpload = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:8080/user/updateprofilepicture",
        formData,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8080/user/", headers).then((res) => {
    ...
    axios.get("http://localhost:8080/user/posts", headers).then((res) => {
      let json = res.data.data;
      console.log(json);
      setUserPosts(json);
    });
