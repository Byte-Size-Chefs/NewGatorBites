...
export default function PostCard(props: { post: Post; color: string }) {
  ...
  const fetchUserDetails = () => {
    const username = typeof props.post.user === 'string' ? props.post.user : props.post.user.username;
  ...
  const clearUpvotes = () => {
    ...
    if (token !== null) {
      ...
      axios
        .patch(`http://localhost:8080/user/clearrating/${props.post.id}`, {}, headers)
        .then((res) => {
          if (res.status === 200) {
  ...
  const handleUpvote = () => {
    ...
    if (token !== null) {
      const headers = { headers: { 'Authorization': `Bearer ${token}` } };
      axios
        .patch(`http://localhost:8080/user/likepost/${props.post.id}`, {}, headers)
        .then((res) => {
          if (res.status === 200) {
  ...
  const handleDownvote = () => {
    ...
    if (token !== null) {
      const headers = { headers: { 'Authorization': `Bearer ${token}` } };
      axios
        .patch(`http://localhost:8080/user/dislikepost/${props.post.id}`, {}, headers)
        .then((res) => {
          if (res.status === 200) {
            setUpvotes(upvotes - 1);
          } else {
      ...
      <div className='relative m-2'>
        <div className="flex items-center">
          {userDetails && ( // Check if userDetails is not null
            <img
            src={userDetails.image_url || 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png'} 
          ...
          <div>
            ...
            <span className='text-sm text-gray-500'> made by {typeof props.post.user === 'string' ? props.post.user : props.post.user.username}</span>
          </div>
        </div>
